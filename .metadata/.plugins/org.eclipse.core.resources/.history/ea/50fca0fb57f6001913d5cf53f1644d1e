...
public class HackathonApplicationTests {
	...
	public void testApproveLoginFailure() throws URISyntaxException {
	  ...
	  final String baseUrl = "http://localhost:"+randomServerPort+"/user/validateRegistrationForm/"+un+"/11111";
	  logger.info(baseUrl);
	  URI uri = new URI(baseUrl);
	  
	  ResponseEntity<Integer> result = restTemplate.getForEntity(uri, Integer.class); //Verify request succeed
	  logger.info(result.getStatusCodeValue());
	  Assert.assertEquals(200, result.getStatusCodeValue());
	  ...
	  @Test public void testSaveCustomer() throws URISyntaxException {
	  RestTemplate restTemplate = new RestTemplate();
	  
	  final String baseUrl ="http://localhost:"+randomServerPort+"/address/saveCustomer/satya/2/2"; URI
	  uri = new URI(baseUrl);
